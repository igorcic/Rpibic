...
consulta_discenter <- consulta_aluno %>% filter(!is.na(`Espaço de estudo blocos de sala`))

consulta_discenter$"EÓtima" <- NA
consulta_discenter$"EÓtima" <- ifelse(str_detect(consulta_discenter$`Espaço de estudo blocos de sala`, 
                                                 "Ótima"), 1, 0)
consulta_discenter$"EDBoa" <- NA
consulta_discenter$"EBoa" <- ifelse(str_detect(consulta_discenter$`Espaço de estudo blocos de sala`, 
                                               "Boa"), 1, 0)

consulta_discenter$"ENão utilizei o local/serviço este ano." <- NA
consulta_discenter$"ENão utilizei o local/serviço este ano." <- ifelse(str_detect(consulta_discenter$`Espaço de estudo blocos de sala`, 
                                                                                  "Não utilizei o local/serviço este ano."), 1, 0)

consulta_discenter$"ERuim" <- NA
consulta_discenter$"ERuim" <- ifelse(str_detect(consulta_discenter$`Espaço de estudo blocos de sala`, 
                                                "Ruim"), 1, 0)

consulta_discenter$"EPéssima" <- NA
consulta_discenter$"EPéssima" <- ifelse(str_detect(consulta_discenter$`Espaço de estudo blocos de sala`, 
                                                   "Péssima"), 1, 0)


quadrodisgestaor5 <- consulta_discenter %>% 
  mutate(Classe = "Espaço de estudo blocos de sala")

quadrodisgestaor5 <- quadrodisgestaor5%>%
  group_by(Classe) %>% 
  summarise(Ótima = percent(sum(`EÓtima`, na.rm = T)/length(`EÓtima`)),
             Boa = percent(sum(`EBoa`, na.rm = T)/length(`EBoa`)),
             "Não utilizei o local/serviço este ano." = percent(sum(`ENão utilizei o local/serviço este ano.`, na.rm = T)/length(`ENão utilizei o local/serviço este ano.`)),
             Ruim = percent(sum(`ERuim`, na.rm = T)/length(`ERuim`)),
             Péssima = percent(sum(`EPéssima`, na.rm = T)/length(`EPéssima`)))
